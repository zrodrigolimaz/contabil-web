...
@Component({
  ...
  template: `
    <div role="group" [attr.aria-labelledby]="idRotulo" [formGroup]="grupo()" class="flex flex-col">
      <span [id]="idRotulo" class="mb-1.5 text-[11.5px] font-semibold text-petrol-800">
        {{ rotulo() }}
      </span>
      ...
      <div class="grid grid-cols-2 gap-2.5">
        <div class="flex flex-col gap-1">
          ...
          <!--
            ...
            NumberValueAccessor pelo seletor input[type=number], e com o tipo vindo de
            um binding o controle receberia texto em vez de número.
          -->
          @if (tipo() === 'data') {
            <input
              [id]="idDe"
      ...
      @if (estado.mensagem(); as texto) {
        <p [id]="idErro" role="alert" class="mt-1.5 text-[11px] font-medium text-danger">
          {{ texto }}
        </p>
      }
    </div>
